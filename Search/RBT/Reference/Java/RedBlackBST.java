...
// Methods for tree drawing
   	public void InOrderPrint(Node p, int level)
   	{
   		if (p != null) {
            InOrderPrint(p.right, level + 1);
            for (int i = 0; i < level; ++i)
                StdOut.print("\t");
            StdOut.println(p.key + "[" + p.value + "]" + (isRed(p) ? "*" : ""));
            InOrderPrint(p.left, level + 1);
        }
   	}
    public void InOrderPrint()
    {
    	StdOut.println("The Red Black Tree is: ");
        if (root != null)
            InOrderPrint(root, 0);
        else
